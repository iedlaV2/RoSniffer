    ...
    if not boolmindmax:
        try:
            if not os.path.exists(mindmaxfilepath):
                ...
                filename_start = mindmaxfetchlink.find('filename=') + len('filename=')
                filename_end = mindmaxfetchlink.find('&', filename_start)
                download_filename = mindmaxfetchlink[filename_start:filename_end]

                download_target_path = os.path.join(mindmaxpath, download_filename)
...
def fetch_serverid(place_id: int) -> list[str]:
    ...
    try:
        ...
        print(f"Network error fetching server IDs for place_id")
        return []
    except Exception as e:
